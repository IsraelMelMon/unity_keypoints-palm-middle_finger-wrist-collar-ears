...
public sealed class BlazePoseSample : MonoBehaviour
{
    ...
    void DrawJoints(Vector3[] joints)
    {
        ...
        Vector3 colarJoint = worldJoints[11]*0.5f+worldJoints[12]*0.5f ;
        // Draw
        for (int i = 0; i < worldJoints.Length; i++)
        {
            
            //Collar
            draw.Cube(colarJoint,0.2f);

            // Muñecas
            //draw.Cube(worldJoints[15], 0.2f);            
            //draw.Cube(worldJoints[16], 0.2f);
            // Orejas
            //if (worldJoints[7].z > worldJoints[8].z *1.2f)
            Debug.Log(worldJoints[7]);
            draw.Cube(worldJoints[7], 0.2f);
            draw.Cube(worldJoints[8], 0.2f);
